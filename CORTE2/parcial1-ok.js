// Maneja la visibilidad de los campos en función del tipo de dimensión seleccionado
...
class Cartesiana {
    ...
    dibujar(ctx, radio, color, nlados) {
        ...
        if (!isNaN(nlados) && nlados >= 3) {
            dibujarPoligono(ctx, 0, 0, radio, nlados);
        } else {
...
function dibujarFigura() {
    ...
    if (tipoDimension === 'lados') {
        ...
        const radio = 100; // Valor predeterminado del radio (puede ser calculado de otra forma si es necesario)
        if (coordenadaTipo === 'cartesiana') {
            ...
            coordenadasCartesiana.dibujar(ctx, radio, color, nlados);
    ...
    } else if (tipoDimension === 'apotema') {
        const apotema = parseFloat(document.getElementById('apotema').value);
        const nlados = parseInt(document.getElementById('nlados').value);
        const radio = calcularRadio(apotema, nlados);
        if (coordenadaTipo === 'cartesiana') {
            const coordX = parseInt(document.getElementById('coordX').value);
            const coordY = parseInt(document.getElementById('coordY').value);
            const coordenadasCartesiana = new Cartesiana(coordX, coordY);
